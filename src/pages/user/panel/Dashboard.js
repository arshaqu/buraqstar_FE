...
const actionCards = (t, navigate, handleLogout) => [
  {
    ...
    icon: (
      <PiClipboardText size={42} sx={{ fontSize: 200, color: "#9ca3af" }} />
    ),
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-5 poppins">
        {cards.map((card, i) => (
          <button
            ...
            className="flex flex-col items-center justify-center gap-3 md:p-10 lg:p-10 p-5 border-2 border-gray-300 rounded-2xl hover:border-[#2858A3] hover:shadow-md transition-all duration-200 bg-white cursor-pointer group"
          >
            <span className="group-hover:scale-110 text-gray-500 transition-transform duration-200">
              {card.icon}
            </span>
            <Typography
              variant="body2"
